...
from django.db.models import Max, Q, OuterRef, Exists, Value, Subquery, Prefetch
...
def buscar_servicios_filtrados(filtros):
    """
    Filtra sobre Servicio y prefetch de PacientexServicio.
    Esto permite mostrar servicios con o sin paciente.
    """
    ...
    if filtros.get('fecha_inicio'):
        qs = qs.filter(fecha__date__gte=filtros['fecha_inicio'])

    if filtros.get('fecha_fin'):
        qs = qs.filter(fecha__date__lte=filtros['fecha_fin'])
    ...
    if filtros.get('servicio_realizado'):
        qs = qs.filter(
            tipo_servicio_realizado__descripcion__icontains=filtros['servicio_realizado']
        )

    # Filtros que afectan al paciente (solo aplican si el servicio tiene pacientes)
    if filtros.get('paciente'):
        paciente = filtros['paciente'].strip()
        if paciente.isdigit():
            qs = qs.filter(pacientexservicio_set__clave=int(paciente))
        else:
            qs = qs.filter(pacientexservicio_set__nombre_completo__icontains=paciente)

    if filtros.get('enfermedad'):
        qs = qs.filter(pacientexservicio_set__enfermedad__nombre__icontains=filtros['enfermedad'])

    if filtros.get('hospital'):
        qs = qs.filter(pacientexservicio_set__hospital__nombre__icontains=filtros['hospital'])

    if filtros.get('ambulancia'):
        qs = qs.filter(pacientexservicio_set__ambulancia__descripcion__icontains=filtros['ambulancia'])

    if filtros.get('base'):
        qs = qs.filter(pacientexservicio_set__base__clave__icontains=filtros['base'])
