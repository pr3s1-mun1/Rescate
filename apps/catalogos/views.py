...
def add_paramedico(request, tipo):
    if tipo != 'paramedicos':
        return redirect('home')  # o manejar otros catálogos

    if request.method == 'POST':
        form = ParamedicoForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('catalogo_general', 'paramedicos')  # tu vista de listado
    else:
        form = ParamedicoForm()

    return render(request, 'add/add_paramedicos.html', {
        'form': form,
        'accion': 'Crear',
        'catalogo': tipo
    })
...
# Editar
def update_paramedico(request, tipo, clave):
    if tipo != 'paramedicos':
        return redirect('home')

    paramedico = get_object_or_404(Paramedicos, clave=clave)

    if request.method == 'POST':
        form = ParamedicoForm(request.POST, instance=paramedico)
        if form.is_valid():
            nuevo_valor_clave = form.cleaned_data['clave']
            if nuevo_valor_clave != paramedico.clave:
                # Crear un nuevo objeto con la nueva clave
                paramedico.pk = nuevo_valor_clave
                paramedico.save(force_insert=True)  # Inserta como nuevo registro
                # Eliminar el antiguo si quieres reemplazarlo
                # Paramedicos.objects.filter(clave=clave).delete()
            else:
                form.save()  # Actualiza normalmente
            return redirect('catalogo_general', 'paramedicos')
    else:
        form = ParamedicoForm(instance=paramedico)

    return render(request, 'updt/updt_paramedicos.html', {
        'form': form,
        'accion': 'Editar',
        'catalogo': tipo
    })
